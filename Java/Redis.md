...
## 缓存击穿
在某一时刻，一个 key 失效之后，多个并行请求来访问这一个 key，就导致多个线程进行了缓存重建操作。
常见解决方式：
![](https://cdn.jsdelivr.net/gh/fynism/Picogo@main/img/20260323190011157.png)
- 使用**互斥锁**保证只有一个线程进行缓存重建操作。
- **逻辑过期**操作，在缓存中以逻辑形式存储时间戳。在重建缓存的时候开启一个子线程。

**互斥锁**业务逻辑：
![](https://cdn.jsdelivr.net/gh/fynism/Picogo@main/img/20260323194501600.png)