...
# 中间件
是一个全局性的 filter。它可以在**每一个** 特定请求 (比如下面给出的例子就是 `http` 请求，实际操作中也确实基本上只会出现 `http` 类型的中间件 ) 的前后进行一些操作，并且返回相应结果。
对，是“每一个”，只要是某协议的请求，都会执行。甚至在返回像 404 这种错误状态码的时候也会执行中间件。
...
首先我定义了一个公共逻辑函数。然后在 `/userinfo` 路径下的函数参数中使用 `Depend()` 来注入这个公共逻辑函数。这样，就能够在访问相应的路径的时候，去执行定义的公共逻辑函数。
