...
- 释放锁时，重入次数-1 。直到重入次数为 0，删除锁。
	...
	2. **可重试**
*当线程获取锁失败时，它不会立即报错返回，而是在一定时间内不断尝试重新获取，直到成功或超时。*
...
因为原本的业务逻辑中，每一次秒杀下单都需要和数据库进行交互，导致在高并发的情况下，处理速度慢、性能低。

所以想到，可以线在 Redis 中把秒杀券相关的信息缓存一下，在 Redis 里面进行秒杀资格的校验，然后再**异步**进行数据库的写入操作。
为了保证**秒杀资格校验**（包括 *判断库存、判断用户是否正在 Set 中、扣减库存*）操作的原子性，这里使用 Lua 脚本来写。
<img src="https://cdn.jsdelivr.net/gh/fynism/Picogo@main/img/20260331165257011.png"/>
