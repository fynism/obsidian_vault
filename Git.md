...
如果团队里每个人都乱拉分支、乱起名字，代码肯定会乱套。所以大公司通常会遵守一套规范（最经典的是 Git Flow）。
一个标准的现代化团队通常这样管理分支：

1. **main (主分支)**
    - **地位**：神圣不可侵犯。
    - **作用**：只存放随时可以发布给用户使用的、最稳定的线上代码。**绝对不能直接在上面写代码**。
2. **dev / develop (开发分支)**
    - **地位**：日常开发的大本营。
    - **作用**：所有人做完功能后，都会合并到这里。测试人员主要测试这个分支的代码。
3. **feature/xxx (功能分支)**
    - **地位**：打工人实际写代码的地方。
    - **作用**：比如你要开发登录功能，就从 dev 拉出一个 feature/login 分支。开发完、测试没问题后，合并回 dev，然后删掉自己。
4. **hotfix/xxx (紧急修复分支)**
    - **地位**：救火队员。
    - **作用**：如果线上（main）突然发现严重 Bug，直接从 main 拉一个 hotfix 分支，修好后立刻合并回 main 和 dev 进行发布。
